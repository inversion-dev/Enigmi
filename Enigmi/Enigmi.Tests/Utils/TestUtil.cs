...
public static class TestUtil
{
        ...
        IClusterClient clusterClient)
    {
        ...
        await TestUtil.Retry(5, 90, async () =>
        {
            ...
            if (response!.Result!.PuzzleCollections.Any())
            {
                puzzleCollectionDto = response!.Result!.PuzzleCollections.First();
                return true;
            }
            return false;
        });
    ...
    public static async Task<(IUserWalletGrain, IOrderGrain)> SimulatePlacingOrderAndWaitForOrderToComplete(IClusterClient clusterClient, string stakeAddress = "stake_address_xxxx4", int quantityToBuy = 1)
    {
        var paymentAddress =
        ...
        var puzzleCollection = await TestUtil.WaitForCollectionToBePublished(clusterClient);
